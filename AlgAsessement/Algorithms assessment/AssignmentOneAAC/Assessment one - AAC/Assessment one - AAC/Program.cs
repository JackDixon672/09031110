...
namespace Assessment_one___AAC
{
    class Program
    {
        ...
        {
            ArrayOptions(); //Providing the user with options pertaining to the array(s) to be analysed
            Numbers numList = new Numbers(numberArray, numberArray.Length, reverse);
            SortingOptions(numList); //Providing the user with options pertaining to the way in which the arrays are sorted

            //Displaying the sorted array
            Console.WriteLine("Would you like to view the array? OPTIONS: (Y/N)");
            if (Console.ReadLine() == "Y") displayArray(numList);

            SearchingOptions(numList); //Providing the user with options pertaining to the way in which the arrays are searched
        }  
        ...
        static void ArrayOptions()
        {
            ...
            switch (merge)
            {
                case (true):
                    ...
                    do
                    {
                        Console.WriteLine("Select an array to be merged \n1 netOne256\n2 netTwo256\n3 netThree256\n4 netOne2048\n5 netTwo2048\n6 netThree2048");
        ...
        {
            //Allowing the user to enter a search item
            Console.WriteLine("Enter a value to search for:");
            int searchItem = int.Parse(Console.ReadLine());

            //Allowing the user to choose which searching algorithm to be used
